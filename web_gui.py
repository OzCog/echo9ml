...
def generate_echo_network():
    ...
    try:
        ...
        if not echo_graph.nodes():
            ...
            # Draw nodes
            nodes = nx.draw_networkx_nodes(echo_graph, pos,
                                         node_size=node_sizes,
...
def toggle_hyper_drive():
    ...
    try:
        ...
        return jsonify({'success': True,
                       'message': f'Hyper drive {"activated" if new_state else "deactivated"}'})
    except Exception as e:
...
def api_inject_random_echo():
    try:
        try:
            ...
            logger.warning("Could not inject random echo: %s", str(e))
...
def api_propagate_echoes():
    try:
        ...
        logger.error("Error in propagate_echoes: %s", str(e))
        return jsonify({'success': False, 'error': str(e)})
